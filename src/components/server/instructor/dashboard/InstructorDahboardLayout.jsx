...
import { InstructorData } from "constants/menuData"; // Adjusted import path
...
const AdminDashboardLayout = ({ children }) => {
  ...
  useEffect(() => {
    const updateMenuState = () => {
      ...
      InstructorData.forEach((menuItem) => {
          ...
          <div className="relative">
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg dropdown border-2 border-gray-100 p-2 z-50">
                <span className="block w-full text-left px-4 py-2 text-gray-800 hover:bg-primarygold hover:text-white rounded-lg">
                  <span className="text-goldlight">Hi!</span>{" "}
                  {user ? user.firstname.toUpperCase() : "User"}
                </span>
                <Link href="/admin/dashboard">
                  <p className="block px-4 py-2 text-gray-800 hover:bg-primarygold hover:text-white rounded-lg">
                    Dashboard
                  </p>
                </Link>
                {/* <Link href="">
                ...
                <button
                  onClick={onLogout}
                  className="block w-full text-left px-4 py-2 text-gray-800 hover:bg-primarygold hover:text-white rounded-lg"
                >
                  Logout
                </button>
              </div>
      ...
      <div className="flex flex-grow">
        ...
        >
          <nav className="text-sm">
            ...
            {InstructorData.map((menuItem, index) => (
              <div key={index}>
                {menuItem.submenu ? (
                  <>
                    <button
