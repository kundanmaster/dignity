...
const getUser = async (id) => {
    try {
      console.log(id);
      const response = await fetch(`/apiRoutes/manage-admin?id=${id}`); // Send id as query parameter
      if (!response.ok) {
      ...
      return { error: "Error fetching admin" };
    }
  };

const AddNewAdmin = ({ params }) => {
  const tabs = [
    "Basic info",
    "Login Credentials",
    "Social Information",
...
const [adminData, setAdminData] = useState();
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
        const adminId = id; // Replace with how you store courseId
        console.log(adminId);
        // Send updated formData to backend endpoint
        console.log(formData);
        // const response = await updateCourse(adminId, formData);
        // console.log("Course updated successfully:", response);
      const response = await updateAdmin(adminId, formData);
      toast.success("Admin updating successfully")
      console.log("Admin updating successfully:", response);
